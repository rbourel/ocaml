...
let rec eval (fm : formula) (v : valuation) : bool =
  match fm  with
    ...
    | Imp(f1,f2) -> if(eval f1 v = false) then true else eval f2 v
...
let rec atoms (fm : formula) : string list =
   match fm  with
    ...
    | P(x) -> [x]
    | Not(f) -> atoms f
    | And(f1,f2) -> uniq ( atoms f1 @ atoms f2)
    | Or(f1,f2) -> uniq( atoms f1 @ atoms f2 )
    | Imp(f1,f2) ->  uniq (  atoms f1 @ atoms f2)
    | Iff(f1,f2) -> uniq ( atoms f1 @ atoms f2 )
...
let rec tautology (fm : formula) : bool =
  let rec aux l v = match l with
    | [] -> eval fm v
    | p :: r -> aux  r (v @ [p,true]) && aux r (v @ [p,false])
  in aux (atoms fm) []
